...
import Ionicons from "react-native-vector-icons/Ionicons"; // ADDED: JM 03-21-2025
...
const LibraryScreen: React.FC = () => {
  ...
  const procedureName = Array.isArray(searchParams.procedureName)
    ? searchParams.procedureName[0]
    : searchParams.procedureName;
  ...
  useEffect(() => {
    ...
    if (alwaysDoParam) {
      setAlwaysDo(
        Array.isArray(alwaysDoParam) ? alwaysDoParam[0] : alwaysDoParam
      );
    }
    if (watchForParam) {
      setWatchFor(
        Array.isArray(watchForParam) ? watchForParam[0] : watchForParam
      );
    }
    if (neverDoParam) {
  ...
  useEffect(() => {
    if (authorizationCode) {
      console.log(
        "Calling getProcedureList with authorization code:",
        authorizationCode
      ); // Debugging statement
      getProcedureList();
    } else {
  ...
  const navigateToviewProcedure = (procedure: any) => {
    ...
    router.push({
      // pathname: "procedureReviewSummary",  // COMMENTED: JM 03-24-2025
  ...
  const getProcedureList = async () => {
    ...
    try {
      ...
      const url = `https://PrefPic.com/dev/PPService/GetProcedureList.php?DeviceID=${encodeURIComponent(
        deviceID.id
            ...
            <ScrollView style={{ flex: 1 }}>
              {procedures.map((procedure, index) => (
                <TouchableOpacity
                  ...
                  onPress={() => navigateToviewProcedure(procedure)}
                >
                  <Text style={styles.procedureNameButtonText}>
                    {procedure.name}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  procedureContainer: {
    ...
    backgroundColor: "#FFFFFF", // ADDED: JM 03-21-2025
    borderColor: "#E0E0E0", // ADDED: JM 03-21-2025
    borderWidth: 1, // ADDED: JM 03-21-2025
    paddingHorizontal: 20, // ADDED: JM 03-21-2025
    borderRadius: 8, // ADDED: JM 03-21-2025
    marginBottom: 5, // ADDED: JM 03-21-2025
    shadowColor: "#000", // ADDED: JM 03-21-2025
    shadowOffset: { width: 0, height: 2 }, // ADDED: JM 03-21-2025
    shadowOpacity: 0.1, // ADDED: JM 03-21-2025
    shadowRadius: 4, // ADDED: JM 03-21-2025
    elevation: 3, // ADDED: JM 03-21-2025
    padding: 18, // ADDED: JM 03-21-2025
    width: "100%", // ADDED: JM 03-21-2025
  },
  item: {
    color: "gray",
    fontSize: 18,
  ...
  procedureButton: {
    ...
    // width: 40, // Set width for the circle                  // MODIFIED: JM 03-25-2025
    // height: 40, // Set height for the circle                // MODIFIED: JM 03-25-2025
  ...
  addButton: {
    ...
    borderRadius: 30,
    alignItems: "center",
    marginBottom: 10, // ADDED: JM 03-25-2025
    justifyContent: "center",
  },
  input: {
    flexDirection: "row",
